...
def mouseClick(x, y, button, pressed):
        ...
        if calibrating and pressed:
            x, y = mouse.position
            clickLoc.append((x, y))
            if len(clickLoc) == len(diceKeys) + 1 + NUMSCORETYPES:
                mouseListener.stop()
...
def keyPress(key):
    ...
    if key == pauseResumeKey:
        ...
        print("'q' - Further info. After pressing q, click dice from top left to bottom right, then the roll button, \nand then the selection boxes from top left to bottom right, then press q again.")
        ...
        if key == keybindsKey:
            ...
            if key == rollKey:
                ...
                for i in range(1, catNums[scoreTab]+1):
                    if key == KeyCode(char=str(i)) or (catNums[scoreTab] == 10 and key == KeyCode(char='0')):
                        if i == 0:
                            val = 10
                        else:
                            val = i
...
print("If playing on a 1080p screen, the saved calibrations should be good.")
print("CONTROLS")
...
print("Current Tab: Basic Tallies")
